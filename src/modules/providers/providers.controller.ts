...
export const listProviders = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { providers, page, limit, total } = await providersService.listProviders(req.query as any);
...
export const updateMyProfile = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const provider = await providersService.updateMyProfile(req.user!.profileId!, req.body);
    sendSuccess(res, { provider });
  } catch (err) { next(err); }
};

export const uploadPortfolioImages = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const images = await providersService.addPortfolioImages(req.user!.profileId!, uploaded);
    sendSuccess(res, { uploadedImages: images }, 201);
  } catch (err) { next(err); }
};

export const deletePortfolioImage = async (req: Request, res: Response, next: NextFunction) => {
  try {
    await providersService.deletePortfolioImage(req.user!.profileId!, req.params.imageId);
...
export const getMyInquiries = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { inquiries, page, limit, total } = await providersService.getMyInquiries(req.user!.profileId!, req.query);
...
export const replyToInquiry = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const inquiry = await providersService.replyToInquiry(req.user!.profileId!, req.params.inquiryId, req.body);
...
export const getMyReviews = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { reviews, page, limit, total } = await providersService.getMyReviews(req.user!.profileId!, req.query);
...
export const replyToReview = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const review = await providersService.replyToReview(req.user!.profileId!, req.params.reviewId, req.body.reply);
