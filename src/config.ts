...
export const config = {
  ...
  whatsapp: {
    ...
    // URL of the WhatsApp bot's HTTP server (for sending outbound messages)
    botBaseUrl: optional('BOT_BASE_URL', 'http://localhost:3000'),
    // Shared secret for internal bot↔backend communication
  ...
  // How many bcrypt rounds to use. 10 = ~100ms (fast enough, still secure).
  // Original code used 12 (~400ms) — this is the main cause of slow auth.
